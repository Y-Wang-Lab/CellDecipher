...
def repeatmask(sequence,dnasource='mouse'):
    ...
    params = {'sequence':sequence,
                'engine':'wublast',    #used to use wublast, switch to rmblast because of errors
                'speed':'default',
                'dnasource':dnasource,
                'ReturnFormat':'links',
                'ReturnMethod':'html'}
    ...
    #req = urllib.request.Request(baseURL+actionURL, data) # POST request (orignal but stopped working )
    url = baseURL + actionURL + "?" + urllib.parse.urlencode(params) # swith to GET request
