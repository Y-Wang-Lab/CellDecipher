...
def calcOligoCost(tiles,pricePerBase=0.11):
	total = 0.0
	for tile in tiles:
...
def probe_design(file_path, output,targetName = "target",verbose = True,species = 'mouse',genomemask = True,
		 repeatmask = True,channel = "B1",num_hits_allowed = 1,maxProbes=100,dTmMax = 5.0,idt_output=False,calcPrice=True,
		 minGC = 38.0, maxGC = 62.0, targetGC = 50.0, minGibbs = -70.0,maxGibbs = -50.0,numOverlap=10):
	...
	tiles = scanSequence(mySeq['sequence'],mySeq['name'],tileStep=1,tileSize=tileSize) # Here we remove masked sequences and rev comp for tiles.
	utils.eprint(f'{len(tiles)} tiles available of length {tileSize}...')
	...
	utils.eprint(f'\nSelecting top {maxProbes} tiles based on distance to targetGibbs = {targetGibbs}')
	bestTiles = []
	while len(bestTiles) < maxProbes and len(tiles) > 0:
	...
	outputTable(bestTiles,outHandle=outhandle)
	################
	# Print out results
	################
	# for tile in bestTiles:
	...
	if idt_output is not None:
		outputIDT(bestTiles,outHandle=outhandle_idt)
