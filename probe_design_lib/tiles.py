...
class Tile:
	...
	def overlaps(self,b, numOverlap):
			"""Return true if b overlaps self"""
			if (self.start <= b.start and b.start <= (self.end-numOverlap)) or (self.start >= b.start and self.start <= (b.end-numOverlap)):
				return True
			else:
				return False
	...
	def makeBarcode(self,channel):
		self.P1 = channel["odd"]+self.threePrimeSeq
		self.P2 = self.fivePrimeSeq + channel["even"]
